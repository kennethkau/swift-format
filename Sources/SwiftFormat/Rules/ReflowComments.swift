...
public final class ReflowComments: SyntaxFormatRule {
  ...
  /// Whether `FileHeader` owns the file's leading comment block: that rule is enabled and a
  /// non-empty template is configured.
  private var fileHeaderIsConfigured: Bool {
    let configuration = context.configuration
    return configuration.rules[FileHeader.self.ruleName] == true
      && !(configuration.fileHeader.template ?? "").isEmpty
  }
  ...
  public override func visit(_ token: TokenSyntax) -> TokenSyntax {
    ...
    // The file's leading comment block belongs to `FileHeader` when that rule is configured;
    // reflowing it would fight the header template's own line structure.
    if token.previousToken(viewMode: .sourceAccurate) == nil, fileHeaderIsConfigured {
      return token
    }
    ...
    while index < trivia.count {
      ...
      // Leave an unmerged comment exactly as written; only rebuild when something merged. The
      // merged text is trimmed of trailing whitespace so the rule's own output is canonical.
      output.append(
        lastMerged == index
          ? trivia[index] : TriviaPiece(commentKind: kind, content: content.trimmingTrailingWhitespace())
      )
      index = lastMerged + 1
    }
  ...
  private struct ParagraphTagger: MarkupWalker {
    ...
    // Explicit initializer: the private stored property would otherwise make the memberwise
    // initializer private, which stable toolchains reject at the call site in `commentRun`.
    init(run: [CommentLine]) {
      self.run = run
    }
  ...
  private func joinableCommentIndices(in trivia: Trivia) -> Set<Int> {
    ...
    while runStart < trivia.count {
      // Only runs of kinds the configuration reflows can ever join; skipping the others avoids
      // parsing them as Markdown for nothing.
      guard let kind = trivia[runStart].commentKind, let configKind = kind.configKind,
        kindsToReflow.contains(configKind)
      else {
        runStart += 1
        continue
      }
      ...
      // The run always contains at least the comment at `runStart`.
  ...
  /// `start`, and tags each line with the joinable paragraph it belongs to. The returned run
  /// always contains at least one line: the comment at `start`.
  ...
  /// The comment's text after its delimiter and one following space. Only one space is
  /// stripped: extra leading spaces can be meaningful formatting, for example an indented code
  /// block:
  ///
  ///     foo()
  ///     bar()
  private func contentBody(_ piece: TriviaPiece, kind: Comment.Kind) -> Substring {
  ...
  private func lineContent(_ piece: TriviaPiece, kind: Comment.Kind) -> String {
    return String(contentBody(piece, kind: kind))
  }
  ...
  /// Whether a line's content is ordinary prose that may be reflowed, as opposed to a divider,
  /// banner, or line protected by the configured prefix rules.
  private func isReflowableProse(_ content: String) -> Bool {
    guard !content.isEmpty, !isDividerOrBanner(content), !isDivider(content) else { return false }
  ...
  private func isDividerOrBanner(_ content: String) -> Bool {
  ...
  private func containsFileHeaderSeparator(_ pieces: Trivia) -> Bool {
    for piece in pieces {
      ...
      let body = contentBody(piece, kind: kind)
...
extension TriviaPiece {
  ...
  fileprivate init(commentKind kind: Comment.Kind, content: String) {
    ...
    case .block, .docBlock:
      preconditionFailure("ReflowComments only rewrites line comments")
    }
  }
