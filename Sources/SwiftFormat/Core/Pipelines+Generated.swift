...
extension FormatPipeline {
  ...
  func rewrite(_ node: Syntax) -> Syntax {
    ...
    node = NoEmptyLinesOpeningClosingBraces(context: context).apply(to: node)
    node = NoEmptyTrailingClosureParentheses(context: context).apply(to: node)
    node = NoLabelsInCasePatterns(context: context).apply(to: node)
    node = NoParensAroundConditions(context: context).apply(to: node)
    node = NoVoidReturnOnFunctionSignature(context: context).apply(to: node)
    node = OmitExplicitReturns(context: context).apply(to: node)
    ...
    node = UseLetInEveryBoundCaseVariable(context: context).apply(to: node)
    node = UseShorthandTypeNames(context: context).apply(to: node)
    node = UseSingleLinePropertyGetter(context: context).apply(to: node)
    node = UseTripleSlashForDocumentationComments(context: context).apply(to: node)
    node = UseWhereClausesInForLoops(context: context).apply(to: node)
    return node
  }
}
