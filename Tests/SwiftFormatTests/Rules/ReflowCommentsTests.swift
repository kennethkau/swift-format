...
final class ReflowCommentsTests: LintOrFormatRuleTestCase {
  ...
  func testMergesThreeLineComments() {
    assertFormatting(
      ReflowComments.self,
      input: """
  ...
  func testDoesNotMergeWhenResultExceedsLineLength() {
    assertFormatting(
      ...
      expected: """
        ...
        """,
      findings: [],
      // The merged line is 115 columns, beyond the testing default line length of 100.
    )
  }
  ...
  func testDoesNotMergeFencedCodeBlock() {
    assertFormatting(
      ReflowComments.self,
      input: """
        /// Example:
  ...
  func testDoesNotMergeThematicBreak() {
    assertFormatting(
      ReflowComments.self,
      input: """
        /// Before the break.
  ...
  func testMergesTabIndentedLineCommentsAtTabStops() {
    // Tabs advance to the next tab stop, so " \t" indentation measures 4 columns with
    // `tabWidth` 4; a flat per-tab count would measure 5 columns and refuse this merge at a
    // 46-column limit.
    assertFormatting(
      ReflowComments.self,
      input: """
      ...
      configuration: {
        var config = Configuration.forTesting(enabledRule: ReflowComments.ruleName)
        config.tabWidth = 4
        config.lineLength = 46
        return config
      }()
    )
  }

  func testDoesNotMergeBlockComments() {
    assertFormatting(
      ReflowComments.self,
      input: """
        /* This comment was
  ...
  func testDoesNotReflowLeadingBlockOwnedByFileHeader() {
    // When `FileHeader` is enabled with a template, the file's leading comment block is that
    // rule's to rewrite; reflowing it would fight the template's own line structure. The input
    // matches the configured template, so both rules must leave it untouched.
    assertFormatting(
      ReflowComments.self,
      input: """
        // Header line one
      ...
      findings: [],
      configuration: {
        var config = Configuration.forTesting(enabledRule: ReflowComments.self.ruleName)
        config.rules[FileHeader.self.ruleName] = true
        config.fileHeader.template = "Header line one\nHeader line two"
        return config
      }()
    )
  }
  ...
  func testMergesThreeDocLineComments() {
    assertFormatting(
      ReflowComments.self,
      input: """
